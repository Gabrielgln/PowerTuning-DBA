...
*******************************************************************************************************************************/
/*
--	1) Alterar o PROFILE caso seja necessário.
Substituir:
	@Profile_Name = 'MSSQLServer'

--	2) Apertar F5 para executar todo o script.
*/
...
BEGIN
	...
	-- Seta as variaveis
	SELECT	@Tamanho_Minimo_Alerta_log = 100000		-- 100 MB
...
BEGIN
	...
	DECLARE @Subject VARCHAR(500), @Fl_Tipo TINYINT, @Importance AS VARCHAR(6),@EmailBody VARCHAR(MAX), 
			...
			@ResultadoWhoisactiveHeader VARCHAR(MAX), @ResultadoWhoisactiveTable VARCHAR(MAX), @Espaco_Disco_Parametro INT
...
BEGIN
	...
	DECLARE @Subject VARCHAR(500), @Fl_Tipo TINYINT, @Importance AS VARCHAR(6), @EmailBody VARCHAR(MAX), @Maxsize_Parametro INT,
			@AlertaMDFLDFHeader VARCHAR(MAX), @AlertaMDFLDFTable VARCHAR(MAX), @EmptyBodyEmail VARCHAR(MAX), @EmailDestination VARCHAR(200)
...
BEGIN
	...
	declare @Tempo_Conexoes_Hs tinyint, @Tempdb_Parametro int, @EmailDestination VARCHAR(200), @Tamanho_Tempdb INT
	...
	*******************************************************************************************************************************/
	...
	BEGIN	-- INICIO - ALERTA				
		...
		BEGIN
			...
			*******************************************************************************************************************************/
			...
			SET @AlertaTempdbUtilizacaoArquivoHeader = @AlertaTempdbUtilizacaoArquivoHeader + '<BR /> Conexões com Transação Aberta <BR />' 
			...
			*******************************************************************************************************************************/			              
			SELECT	@Importance =	'High',
					@Subject =		'ALERTA: A Utilização do Arquivo MDF do Tempdb está acima de ' + cast(@Tempdb_Parametro as varchar) + '% no Servidor: ' + @@SERVERNAME,
	...
	BEGIN	-- INICIO - CLEAR		
		...
		BEGIN
			...
			*******************************************************************************************************************************/
			SELECT	@Importance =	'High',
					@Subject =		'CLEAR: A Utilização do Arquivo MDF do Tempdb está abaixo de ' + cast(@Tempdb_Parametro as varchar) + '% no Servidor: ' + @@SERVERNAME,
					@EmailBody =	@AlertaTamanhoMDFTempdbHeader + @EmptyBodyEmail + @AlertaTamanhoMDFTempdbTable + @EmptyBodyEmail +
									@AlertaTempdbUtilizacaoArquivoHeader + @EmptyBodyEmail + @AlertaTamanhoMDFTempdbConexoesTable + @EmptyBodyEmail
...
BEGIN
	...
	-- Declara as variaveis
	DECLARE @EmailBody VARCHAR(MAX), @AlertaConexaoSQLServerHeader VARCHAR(MAX), @AlertaConexaoSQLServerTable VARCHAR(MAX), @EmptyBodyEmail VARCHAR(MAX), @Importance AS VARCHAR(6),
			@Subject VARCHAR(500), @Qtd_Conexoes INT, @Conexoes_SQLServer_Parametro INT, @Fl_Tipo INT, @EmailDestination VARCHAR(200)
...
BEGIN
	...
	-- Recupera os parametros do Alerta
	--------------------------------------------------------------------------------------------------------------------------------
	DECLARE @Queries_Demoradas_Parametro INT, @EmailDestination VARCHAR(200)
...
BEGIN
	...
	-- Recupera os parametros do Alerta
	--------------------------------------------------------------------------------------------------------------------------------
	DECLARE @JobFailed_Parametro INT, @EmailDestination VARCHAR(200)
...
BEGIN
	...
	-- Recupera os parametros do Alerta
	--------------------------------------------------------------------------------------------------------------------------------
	DECLARE @SQL_Reiniciado_Parametro INT, @EmailDestination VARCHAR(200)
...
BEGIN
	...
	-- Recupera os parametros do Alerta
	--------------------------------------------------------------------------------------------------------------------------------
	DECLARE @Database_Criada_Parametro INT, @EmailDestination VARCHAR(200)
...
BEGIN
	...
	-- Recupera os parametros do Alerta
	--------------------------------------------------------------------------------------------------------------------------------
	DECLARE @Database_Sem_Backup_Parametro INT, @EmailDestination VARCHAR(200)
